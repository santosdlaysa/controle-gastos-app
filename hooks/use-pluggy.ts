...
import {
  isPluggyEnabled,
  setPluggyEnabled as storePluggyEnabled,
  getPluggyConnections,
  addPluggyConnection,
...
export function usePluggy() {
  ...
  const syncAndSaveMut = trpc.pluggy.syncAndSave.useMutation();
  ...
  // Sync all connections — saves directly to backend DB
  const syncAll = useCallback(async () => {
    ...
    try {
      ...
      for (const conn of connections) {
        const result = await syncAndSaveMut.mutateAsync({
          itemId: conn.itemId,
          from,
          to,
        });
      ...
      // Invalidate expense queries so UI refreshes
      utils.expense.getByMonth.invalidate();
      utils.history.getSummaries.invalidate();

      const syncTime = new Date().toISOString();
      await setLastSyncTime(syncTime);
      setLastSync(syncTime);
      setSyncResult({ added: totalAdded, skipped: 0 });
    } catch (error) {
      console.error("Pluggy sync error:", error);
    } finally {
      setSyncing(false);
    }
  }, [syncing, connections, syncAndSaveMut, utils]);
